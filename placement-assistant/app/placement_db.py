...
class PlacementDb:
    ...
    def create_application(self, student_id: int, drive_id: int) -> tuple[int, bool]:
        ...
        Part 3.1: a duplicate application is a success, not an error. Today a second insert for the
    ...
    def claim_slot(self, slot_id: int, student_id: int, expected_version: int) -> bool:
        ...
        Part 3.2: optimistic locking. Book only if the slot is still free AND its version still equals
    ...
    def record_notification(self, roll_no: str, message: str, dedupe_key: str) -> tuple[int, bool]:
        ...
        Part 3.3: store it only if no notification has this dedupe_key; otherwise return the existing
    ...
    def once(self, key: str, tool_name: str, effect: Callable[[], dict]) -> tuple[dict, bool]:
        """Part 2.2: run `effect` at most once per key.
